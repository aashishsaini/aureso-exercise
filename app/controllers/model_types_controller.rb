class ModelTypesController < ApplicationController
  ...
  before_action :set_model_type, except: [:index, :new, :create]

  # GET /model_types
  # GET /model_types.json
  def index
    @all_models = Model.all.to_json(include: {model_types: {only: [:name], methods: [:total_price]}}, only: [:name])
    respond_to do |format|
      format.json { render json: {models: JSON.parse(@all_models)} , location: @model_type }
    end
  end
